module.exports = function(STAMPEDE) {
  ...
  auth.initiate = function(app) {
    var host_name = config.hosts[environment]
    var realm = host_name + (
      environment === "production" ? "" : (":"+config.port)) + "/"
    app.googleRealm = realm
    app.googleReturnURL = realm + 'auth/google/return'
    ...
    if (config.allowed_user_emails.indexOf(email) > -1) {
      ...
      db.smembers("stampede_shares", function(redis_errors, share_list) {
        ...
        if (
          ...
          console.log(
            "authentication | per share holder |",
            "email, path, allowed_for_holder:", 
            email, path, allowed_for_holder
          )
        }
        callback(
          (loaded_user.email !== undefined && allowed_for_holder), 
          (loaded_user.email ? loaded_user : null)
        )
      })
    }
  }
